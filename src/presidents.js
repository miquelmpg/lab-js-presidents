...
function getNames(presidentsArr) {
  return presidentsName = presidentsArr.map(president => president.name);
}
...
function getDemocraticPresidents(presidentsArr) {
  return democraticPresidents = presidentsArr.filter(president => president.party === "Democratic");
}
...
function  countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce((countYears, year) => {
    if (year.leftOffice === null) {
      return countYears + 0;
    }
    return countYears + (year.leftOffice - year.tookOffice)}, 0);
}
...
function sortPresidentsByBirthYear(presidentsArr) {
  return presidentsArr.sort((yearA,yearB) => (yearA.birthYear - yearB.birthYear));
}
...
function getAgeAtInauguration(presidentsArr) {
  return presidentsArr.map(president => {
    if (!president.ageAtInauguration) {
      president.ageAtInauguration = 0;
    }
...
function countRepublicanPresidents(presidentsArr) {
  return presidentsArr.reduce((sumPresidents, president) => {
      if (president.party !== "Republican") {
        return sumPresidents + 0;
      }
      return sumPresidents + 1}, 0);
}
...
function sortPresidentsByName(presidentsArr) {
  return presidentsArr.sort((presidentA, presidentB) => {
    if (presidentA.name < presidentB.name) return -1;
    if (presidentA.name > presidentB.name) return 1;
    if (presidentA.name === presidentB.name) return 0;
  });
}